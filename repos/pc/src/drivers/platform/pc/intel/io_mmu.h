                      ...
                      private Translation_table_registry
{
	public:
		               ...
		               public Registered_translation_table
		{
			...
			public:
				...
				void unmap_irqs(Pci::Bdf const & bdf) override
				{
					if (!_intel_iommu.irq_remapping())
						return;

					_irq_table.unmap(_irq_allocator, bdf, [&] (unsigned idx) {
						/* invalidate interrupt entry cache */
						_intel_iommu.invalidator().invalidate_irq(idx, false);
					});
				}

				Irq_info map_irq(Pci::Bdf const & bdf, Irq_info const & info) override
				{
					if (!_intel_iommu.irq_remapping())
						return info;

					return _irq_table.map(_irq_allocator, bdf, info, [&] (unsigned idx) {
	...
	public:
		...
		bool     irq_remapping()        const { return _remap_irqs; }
