	...
	public:
		             ...
		             FN && fn)
		{
			...
			try {
				...
				_entries[_lo_index(idx)] = Irq_remap::lo_val(session_info);

				Utils::clflush(&_entries[_lo_index(idx)]);
				Utils::clflush(&_entries[_hi_index(idx)]);
		...
		template <typename FN>
		void unmap(Irq_allocator & irq_alloc, Pci::Bdf const & bdf, FN && fn)
		{
