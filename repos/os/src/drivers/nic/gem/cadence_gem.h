...
namespace Genode { class Cadence_gem; }

/**
 * Base driver Xilinx EMAC PS module
 */
class Genode::Cadence_gem
:
	...
	public Phyio
{
	private:
		...
		struct Dma_config : Register<0x10, 32>
		{
			...
			{
				enum { BUFFER_1600B = 0x19 };
			};

			static access_t init()
			{
				access_t v = 0;
				Ahb_mem_rx_buf_size::set(v, Ahb_mem_rx_buf_size::BUFFER_1600B);
				Rx_pktbuf_memsz_sel::set(v, Rx_pktbuf_memsz_sel::SPACE_8KB);
				Tx_pktbuf_memsz_sel::set(v, Tx_pktbuf_memsz_sel::SPACE_4KB);
				return;
			}
		...
		void _phy_setup_op(const uint8_t phyaddr, const uint8_t regnum,
		                   const uint16_t data,
		                   const Phy_maintenance::Operation::Type op)
		{
			_mdio_wait();

			/* write mgtcr and wait for completion */
			Phy_maintenance::access_t pm = 0;
			Phy_maintenance::Clause_22::set(pm, 1);
			Phy_maintenance::Operation::set(pm, op);
			Phy_maintenance::Phy_addr::set(pm, phyaddr);
			Phy_maintenance::Reg_addr::set(pm, regnum);
			Phy_maintenance::Must_10::set(pm, Phy_maintenance::Must_10::INIT);
			Phy_maintenance::Data::set(pm, data);
			write<Phy_maintenance>(pm);
	...
	public:
		 ...
		 */
		Cadence_gem(addr_t const base, size_t const size, const int irq,
		...
		void phy_write(const uint8_t phyaddr, const uint8_t regnum,
		               const uint16_t data)
		{
			_phy_setup_op(phyaddr, regnum, data, Phy_maintenance::Operation::WRITE);
		}

		void phy_read(const uint8_t phyaddr, const uint8_t regnum,
		              uint16_t& data)
		{
		...
		{
			const uint32_t * const low_addr_pointer =
				reinterpret_cast<const uint32_t*>(&mac.addr[0]);

			const uint16_t * const high_addr_pointer =
				reinterpret_cast<const uint16_t*>(&mac.addr[4]);
		...
		{
			Nic::Mac_address mac;
			uint32_t * const low_addr_pointer =
				reinterpret_cast<uint32_t*>(&mac.addr[0]);

			uint16_t * const high_addr_pointer =
				reinterpret_cast<uint16_t*>(&mac.addr[4]);
		...
		virtual void handle_irq(int irq_number)
		{
			...
			if ( Interrupt_status::Rx_complete::get(status) ) {
				...
				while (_rx_buffer.package_available()) {
					...
					/*
					 * TODO use this buffer directly as the destination for
					 * the DMA controller to minimize the overrun errors
					 */
					const size_t buffer_size = _rx_buffer.package_length();
					char * const buffer =
						static_cast<char*>(_rx_buffer_alloc.alloc(buffer_size));

					/*
					 * Copy data from rx buffer to new allocated buffer.
					 * Has to be copied, because the extern allocater possibly
					 * is using the cache.
					 */
					if (_rx_buffer.get_package(buffer, buffer_size) !=
					    buffer_size )
					{
				...
				const uint16_t lost_packages =
					read<Rx_overrun_errors::Counter>();
				...
				PWRN("IRQ %d with unkown reasone recevied (status: %08x).",
				     irq_number, status);
			}
		}
};
