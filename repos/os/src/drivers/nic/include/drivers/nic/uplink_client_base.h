...
class Genode::Uplink_client_base : Noncopyable
{
	protected:
		...
		Signal_handler<Uplink_client_base>  _conn_packet_stream_handler      { _env.ep(), *this, &Uplink_client_base::_conn_handle_packet_stream };
		...
		void _conn_handle_packet_stream()
		{
		...
		void _handle_acks()
		{
			while (_conn->tx()->ack_avail())
				_conn->tx()->release_packet(_conn->tx()->try_get_acked_packet());
		}
		...
		virtual void _custom_handle_packet_stream()
		{
			class Unexpected_call { };
			throw Unexpected_call { };
		}

		virtual bool _custom_packet_stream_handler() { return false; }
