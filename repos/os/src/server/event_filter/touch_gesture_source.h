...
class Event_filter::Touch_gesture_source : public Source, Source::Filter
{
	private:
		...
		struct Gesture : Interface, private Registry<Gesture>::Element
		{
			...
			/*
			 * emits key sequence or key combo from XML
			 */
			void emit_from_xml(Source::Sink &destination, bool release)
			{
				_xml.with_xml_node([&] (Xml_node const &node) {
					_emit_from_xml(destination, node, release); });
			}
		...
		struct Swipe : Gesture
		{
			...
			/* config */
			Rect           const _rect;
			unsigned       const _distance;
			Direction      const _direction;
			Microseconds   const _duration;
			unsigned       const _num_fingers;
			...
			{
				for (unsigned i = 0; i < _num_fingers && i < MAX_FINGERS; i++)
			...
			bool _detected()
			{
				...
				_for_each_finger([&] (Constructible<Finger> & finger) {
					...
					if (finger->distance >= _distance)
						fingers_okay++;

					return true;
				});

				return fingers_okay == _num_fingers;
			}
			...
			void handle_event(Sink &destination, Input::Event const &ev) override
			{
				ev.handle_touch([&] (Input::Touch_id id, float x, float y) {
					...
					{
						case IDLE:
							if (id.value >= _num_fingers)
								return;
							if (_rect.valid() && !_rect.contains(p))
								return;

							_state = State::DETECT;
							_timeout.schedule(_duration);
							[[fallthrough]];
						case DETECT:
							if (id.value >= _num_fingers) {
								cancel();
							...
							if (_detected()) {
								...
								emit_from_xml(destination, false);
							}
				...
				ev.handle_touch_release([&] (Input::Touch_id id) {
					if (id.value == 0) {
						...
						if (_state == TRIGGERED)
							emit_from_xml(destination, true);
						cancel();
					}
				});
			}
			...
			: Gesture(registry, alloc, node), _timer(timer),
			  _rect(Rect::from_xml(node)),
			  _distance(node.attribute_value("distance", 100)),
			  _direction(_direction_from_xml(node)),
			  _duration(node.attribute_value("milliseconds", 1000U)*1000),
			  _num_fingers(node.attribute_value("fingers", 1U))
			{
				if (_num_fingers > MAX_FINGERS)
		...
		struct Hold : Gesture
		{
			...
			unsigned            _num_fingers;
			unsigned            _fingers_present { 0 };

			/* Rect of size _area around the starting touch */
			...
			void handle_event(Sink &destination, Input::Event const &ev) override
			{
				ev.handle_touch([&] (Input::Touch_id id, float x, float y) {
					...
					{
						case IDLE:
							_rect.construct(p - Point { (int)_area.w/2, (int)_area.h/2 }, _area);
							_last_pos = p;
							_state = State::DETECT;
							[[fallthrough]];
						case DETECT:
							if (_fingers_present > _num_fingers || !_rect->contains(p)) {
								cancel();
								return;
							}
							if (!_timeout.scheduled() && _fingers_present == _num_fingers)
			...
			: Gesture(registry, alloc, node), _timer(timer), _trigger(trigger),
			  _area(Area::from_xml(node)),
			  _delay(node.attribute_value("milliseconds", 1000U)*1000),
			  _num_fingers(node.attribute_value("fingers", 1U))
			{
				if (_area.w == 0) _area.w = 30;
				if (_area.h == 0) _area.h = 30;
			}
		};
	...
	public:
		...
		Touch_gesture_source(Owner &owner, Xml_node config,
