...
namespace Genode { namespace Trace {
	...
	class Duration
	{
		...
		public:
			Duration(char const * name, unsigned long long data)
			: _name(name), _data(data)
			{ Checkpoint(_name, _data, Checkpoint::Type::START); }

			~Duration()
			{ Checkpoint(_name, _data, Checkpoint::Type::END); }
	};
...
#define GENODE_TRACE_DURATION(data) \
	Genode::Trace::Duration(__PRETTY_FUNCTION__, (unsigned long long)data);
...
#define GENODE_TRACE_DURATION_NAMED(data, name) \
	Genode::Trace::Duration(name, (unsigned long long)data);
