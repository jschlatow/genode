 ...
 * beginning. Note, that the padding is not present if the entry fits perfectly
 ...
 *
 * One way to approach this could be to split the buffer into two equal
	...
	public:
		...
		class Entry
		{
			...
			public:
				...
				/* return whether data is invalid */
				bool empty()    const { return last() || head() || _padding(); }

				/* entry is head (zero length) */
				bool head()     const { return _entry->head(); }
		};
