...
{
	GENODE_TRACE_DURATION_NAMED(libc_fd, socket_fs_getpeername);
...
{
	GENODE_TRACE_DURATION_NAMED(libc_fd, socket_fs_getsockname);
...
{
	GENODE_TRACE_DURATION_NAMED(libc_fd, socket_fs_accept);
...
{
	GENODE_TRACE_DURATION_NAMED(libc_fd, socket_fs_bind);
...
{
	GENODE_TRACE_DURATION_NAMED(libc_fd, socket_fs_connect);
...
{
	GENODE_TRACE_DURATION_NAMED(libc_fd, socket_fs_listen);
...
{
	GENODE_TRACE_DURATION_NAMED(len, socket_fs_recvfrom);
...
extern "C" ssize_t socket_fs_sendto(int libc_fd, void const *buf, ::size_t len, int flags,
                                    sockaddr const *dest_addr, socklen_t dest_addrlen)
{
	GENODE_TRACE_DURATION_NAMED(len, socket_fs_sendto);
...
{
	GENODE_TRACE_DURATION_NAMED(libc_fd, socket_fs_getsockopt);
...
{
	GENODE_TRACE_DURATION_NAMED(libc_fd, socket_fs_setsockopt);
...
{
	GENODE_TRACE_DURATION_NAMED(libc_fd, socket_fs_shutdown);
...
{
	GENODE_TRACE_DURATION_NAMED(protocol, socket_fs_socket);
...
{
	GENODE_TRACE_DURATION_NAMED(0, getifaddrs);
...
{
	GENODE_TRACE_CHECKPOINT_NAMED(fd, socketfs_plugin_fcntl);
...
{
	GENODE_TRACE_DURATION_NAMED(count, socketfs_plugin_read);
...
{
	GENODE_TRACE_DURATION_NAMED(count, socketfs_plugin_write);
...
bool Socket_fs::Plugin::poll(File_descriptor &fdo, struct pollfd &pfd)
{
	...
	GENODE_TRACE_CHECKPOINT_NAMED(0, socketfs_plugin_poll);
	Socket_fs::Context *context { nullptr };
...
{
	GENODE_TRACE_CHECKPOINT_NAMED(0, socketfs_plugin_supports_select);
...
int Socket_fs::Plugin::select(int nfds,
                              ...
                              timeval *timeout)
{
	GENODE_TRACE_DURATION_NAMED(0, socketfs_plugin_select);
	int nready = 0;
...
{
	GENODE_TRACE_CHECKPOINT_NAMED(fd, socketfs_plugin_close);
	Socket_fs::Context *context = dynamic_cast<Socket_fs::Context *>(fd->context);
	if (!context) return Errno(EBADF);
...
{
	GENODE_TRACE_CHECKPOINT_NAMED(request, socketfs_plugin_ioctl);
	if (request == FIONREAD) {
		/*
