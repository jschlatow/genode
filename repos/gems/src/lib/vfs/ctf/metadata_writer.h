...
#ifndef _METADATA_WRITER_H_
#define _METADATA_WRITER_H_
...
class Ctf::Metadata_writer
{
	...
	public:
		...
		Metadata_writer(Attached_rom_dataspace & metadata_rom, uint64_t freq)
		: _metadata_rom(metadata_rom),
		  _timestamp_freq(freq)
		{ log("Timestamp frequency is ", _timestamp_freq, "MHz"); }

		void write_file(Genode::New_file & dst)
		{
			...
			_with_metadata_rom(
				...
				[&] (char const * start, size_t len) {
					...
					print(output, " = ", _timestamp_freq*1000*1000, ";\n");
				},
